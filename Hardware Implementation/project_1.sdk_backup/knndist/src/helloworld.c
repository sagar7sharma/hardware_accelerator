...
//  char lines[2048]; //line buffer for fgets
      ...
      (f_open(&fdst, "output.txt", FA_WRITE | FA_CREATE_ALWAYS) == FR_OK))
  {
    ...
    for ( i = 0; fgets(buffer11, sizeof buffer11, file11); ++i )
      for ( j = 0; fgets(buffer22, sizeof buffer22, file22); ++j )
      {
         /*
          * Parse the comma-separated values from each line into 'array'.
          */
    	  *(baseaddr_p+129) = 0x40000000;
         for ( k = 0, ptr11 = buffer11, ptr22 = buffer22; j < (sizeof(buffer11)/sizeof(*buffer11)); ++k, ++ptr11, ++ptr22 )
         {
