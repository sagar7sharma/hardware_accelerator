...
int main(void) {
	...
	for (int counter = 0; counter < num_counter; counter++) {
		...
		for (unsigned int i =0; i < number_keypoints1 *64; i++)
			((float *)adesc11)[i] = (float)descriptor[i]; //
                        //for (unsigned int j=0; j < 64; j++)
                                	//adesc11[i][j] = (float)descriptor[i*j];
		...
		for (int img_cnt = 0; img_cnt < img_num; img_cnt++) {
			...
			for (unsigned int i =0; i < number_keypoints2 *64; i++)
				//for (unsigned int j=0; j < 64; j++)
                                	//adesc22[i][j] = (float)descriptor[i*j];
			...
			vector<cv::Point2f> matches_kaze, inliers_kaze;
                        ...
                        float dist0; // most minimum distance
                        float dist1; // next most minimum distance
                        int nidx[2]; // index for the dist0 and dist1 located in key points of Query Image
                        double bufvalue =0; // temporary value variable
                        float bufvalue_ref; // distance between two key points
                        bool switchs = false;                                       
                        for(int i=0; i<number_keypoints1; i++){ // iterate across key points in Train Image
                                bufvalue = 0; // initialize values
                                dist0 = (double) FLT_MAX; 
                                dist1 = (double) FLT_MAX;
                                nidx[0] = -1; 
                                nidx[1] = -1;
                                switchs = false;
                                for(int j=0; j<number_keypoints2; j++){ // iterate across the key points in Query Image
                                        bufvalue = 0.0; 
                                        for(int k=0; k<64; k++){
                                        	float t = adesc11[i][k] - adesc22[j][k];
                                                bufvalue += (float) t*t; 
                                        }
                                        bufvalue_ref = (float)sqrt(bufvalue); // bufvalue_ref has the distance between two key points 
                                 ...
                                 if( nidx[0] > 0){ // dist0 contains the most minimum distance for a given key point in Train Image
                                                   // itrated in all of key points in Query Imagea and dist1 next most minimum distance
                                 	if(dist0 < (0.8f*dist1)) { // check for the threshold
                                        	matches_kaze.push_back(key_points11[i].pt); // satsified then add as a match
