...
__global__ void knnMatchDist(float *adesc11, float *adesc22, float *dist, int number_keypoints2, int number_keypoints1)
                                   {    
                                    	int ref = threadIdx.x + (((gridDim.x * blockIdx.y) + blockIdx.x)*blockDim.x);  
                                    	if(ref < (number_keypoints1*number_keypoints2))
                                        { 
                                        	double bufvalue = 0.0;
                                                int row_ref = (ref / number_keypoints2) * 64; 
                                                int column_ref = (ref % number_keypoints2) *64;
                                                float t;
                                        	for(int k=0; k<64; k++){
                                        		t = adesc11[row_ref +k] - adesc22[column_ref + k];
                                                	bufvalue += t*t; 
                                        	}
                                        	dist[ref] = sqrt(bufvalue); // calculate distance between the key points and store
                                    	}
                                       
                                   }

__global__ void knnMatchMain(float *dist, int number_keypoints2, int number_keypoints1,int *nidx_ref, int *found)
                                   {
                                    	int ref = threadIdx.x + (((gridDim.x * blockIdx.y) + blockIdx.x)*blockDim.x); 
                                        if(ref < number_keypoints1)
                                        {
                                        	float dist0 = (double) FLT_MAX;
                        			float dist1 = (double) FLT_MAX;  
                        			int nidx[2] = {-1,-1};	
                                                int switchs = -1;
                                                for(int j=0; j<number_keypoints2; j++){
                                                	if(dist[ref*number_keypoints2 + j] < dist0 ){
                                        			if(switchs > 0) {
                                        				nidx[1] = nidx[0];
                                                			dist1 = dist0;
                                                		}
                                                		nidx[0] = j;
                                                		dist0 = dist[ref*number_keypoints2 + j];
                                                		switchs = 10;                     	
                                        		}
                                 		}
                                 		if( nidx[0] > 0){
                                 			if(dist0 < (0.8f*dist1)) { // based on the distance obtained see if a match was found and record it
                                        			found[ref] = 10; 
                                                        	nidx_ref[ref] = nidx[0];
                                         			}
                                         		}
...
int main(void) {
	...
	for (int counter = 0; counter < num_counter; counter++) {
		...
		float adesc11[number_keypoints1][64]; // added
		for (unsigned int i =0; i < number_keypoints1 *64; i++)
			((float *)adesc11)[i] = (float)descriptor[i]; //
                float cuda_adesc11[number_keypoints1*64]; // Cuda Mem
                for(unsigned int i=0; i<number_keypoints1; i++)
                	for(unsigned int j=0; j<64; j++)
                        	cuda_adesc11[(i*64)+j] = adesc11[i][j]; //
		...
		/*struct aKeyPoint { //added
				float x;
				float y;
				float size;
			} akey_points11[number_keypoints1];
			for (unsigned int i=0; i<number_keypoints1; i++){
				akey_points11[i].x = key_points[i].x_coord;
				akey_points11[i].y = key_points[i].y_coord;
				akey_points11[i].size = key_points[i].scale;
			}*/
		res_ex_time += tkaze0;
		...
		for (int img_cnt = 0; img_cnt < img_num; img_cnt++) {
			...
			float adesc22[number_keypoints2][64]; // added
			for (unsigned int i =0; i < number_keypoints2 *64; i++)
                        	((float *)adesc22)[i] = (float)descriptor[i]; //

                        float cuda_adesc22[number_keypoints2*64];
                        for(unsigned int i=0; i<number_keypoints2; i++) // Cuda Mem
                		for(unsigned int j=0; j<64; j++)
                        		cuda_adesc22[(i*64)+j] = adesc22[i][j];//
                        int cuda_found[number_keypoints1]; // Cuda Mem
                        int cuda_nidx[number_keypoints1]; // Cuda Mem
                        //float cuda_dist[number_keypoints1*number_keypoints2];
			...
			/*aKeyPoint akey_points22[number_keypoints2]; //added
			for (unsigned int i=0; i<number_keypoints2; i++){
				akey_points22[i].x = key_points[i].x_coord;
				akey_points22[i].y = key_points[i].y_coord;
				akey_points22[i].size = key_points[i].scale;
			} //*/

			            cout << "number of keypoints1: " << number_keypoints1 << "\n";
                        cout << "number of keypoints2: " << number_keypoints2 << "\n";
			/*************************************** KAZE Matching ****************************************/

			// Kaze Matching Time
			//const double t5 = (double)(cv::getTickCount());

			// KAZE Matching Variables/Array
			vector<cv::Point2f> matches_kaze, inliers_kaze;
                        cv::Mat HG;
                         
                        int32_t numBytes = 0; 
                        int *cudaDev_found; // cuda variable to indicate a match was found in a thread
                        int *cudaDev_nidx; // cuda variable to indicate where was the match found
                        float *cudaDev_adesc11; // cuda variable to pass descriptors of key points in Train Image
                        float *cudaDev_adesc22; // cuda variable to pass descriptors of key points in Query Image
                        float *cudaDev_dist; // temporary storage of all distances between the keypoints of Train Image and Query Image
                        numBytes = number_keypoints1*sizeof(int); 
                        cudaMalloc((void**)&cudaDev_found, numBytes); // allocate memory for cudaDev_found
                        cudaMemset(cudaDev_found, -1, numBytes); // intitialize all values to -1 in cudaDev_found
                        cudaMalloc((void**)&cudaDev_nidx, numBytes); // allocate memory for cudaDev_nidx
                        cudaMemset(cudaDev_nidx, -1, numBytes); // initialize all values to -1 in cudaDev_nidx
                        numBytes = number_keypoints1*64*sizeof(float);
                        cudaMalloc((void**)&cudaDev_adesc11, numBytes); // allocte memory for cudaDev_adesc11
                        cudaMemcpy(cudaDev_adesc11, cuda_adesc11, numBytes, cudaMemcpyHostToDevice); // copy descriptors of Train Image to
                                                                       //  cudaDev_adesc11
                        numBytes = number_keypoints2*64*sizeof(float); 
                        cudaMalloc((void**)&cudaDev_adesc22, numBytes); // allocte memory for cudaDev_adesc22
                        cudaMemcpy(cudaDev_adesc22, cuda_adesc22, numBytes, cudaMemcpyHostToDevice); // copy descriptors of Train Image to
                                                                        // cudaDev_adesc22
                        numBytes = number_keypoints1*number_keypoints2*sizeof(float);
                        cudaMalloc((void**)&cudaDev_dist, numBytes); // allocate memory for cudaDev_dist
                        cudaMemset(cudaDev_dist, 0, numBytes); // initialize with 0 for all elements in cudaDev_dist
                        const double t5 = (double)(cv::getTickCount());
                        int product = number_keypoints1*number_keypoints2;
                        knnMatchDist<<<(product/1024),1024>>>( cudaDev_adesc11, cudaDev_adesc22, cudaDev_dist, number_keypoints2, number_keypoints1); // launch number_key_points1*number_key_points2 threads and calculate the distance between the pair of key points
                        cudaError_t err = cudaGetLastError();
			if (err != cudaSuccess) 
    				printf("Error: %s\n", cudaGetErrorString(err));
                        knnMatchMain<<<(number_keypoints1/1024),1024>>>(cudaDev_dist, number_keypoints2, number_keypoints1,cudaDev_nidx,cudaDev_found); // launch number_of_keypoins1 to ascertain for a match
                        err = cudaGetLastError();
                        if (err != cudaSuccess) 
    				printf("Error: %s\n", cudaGetErrorString(err));
                        numBytes = number_keypoints1*sizeof(int);
                        cudaMemcpy(cuda_found, cudaDev_found, numBytes, cudaMemcpyDeviceToHost);
                        if (err != cudaSuccess) 
    				printf("Error: %s\n", cudaGetErrorString(err));
                        cudaMemcpy(cuda_nidx, cudaDev_nidx, numBytes, cudaMemcpyDeviceToHost);
                        if (err != cudaSuccess) 
    				printf("Error: %s\n", cudaGetErrorString(err));
                        const double t6 = (double)(cv::getTickCount());
                        for(unsigned int i=0; i<number_keypoints1;i++){ 
                        	if(cuda_found[i] == 10){ // iterate through cuda_found to find if a match was found
                                	matches_kaze.push_back(key_points11[i].pt); // if match was found then we add the key point pair from the 
                                                                                    // train image and query image, else discard
                                        matches_kaze.push_back(key_points22[cuda_nidx[i]].pt);
                                }
                        }
                        cudaFree(cudaDev_found); // free memory
                        cudaFree(cudaDev_nidx);
                        cudaFree(cudaDev_adesc11);	
                        cudaFree(cudaDev_adesc22);
                        cudaFree(cudaDev_dist);
			// Kaze Matching Time
			//const double t6 = (double)(cv::getTickCount());
