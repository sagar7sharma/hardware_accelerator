...
__global__ void knnMatch(float *adesc11, float *adesc22, int *found, int *nidx_ref, int number_keypoints2, int number_keypoints1)
                                   {    
                        		...
                        		int switchs = -1;
                                        for(int j=0; j<number_keypoints2; j++){
                                        	bufvalue = 0.0; 
                                        	for(int k=0; k<64; k++){
                                        		float t = adesc11[(ref*64) +k] - adesc22[(j*64) + k];
                                                	bufvalue += t*t; 
                                        	}
                                        	bufvalue_ref = (float)sqrt(bufvalue); // bufvalue_ref contains the distance between given keypoint 
                                                                 // of train image and iterated key point of the Query Image
                                        	if(bufvalue_ref < dist0 ){
                                        		if(switchs > 0) {
                                        			nidx[1] = nidx[0];
                                                		dist1 = dist0;
                                                	}
                                 	...
                                 	if( nidx[0] > 0){ // dist0 contains the most minimum distance and dist1 contains the next most minimum
                                                          // dist1 containes the next most minimum distance
                                 		if(dist0 < (0.8f*dist1)) { // check for the match
                                        		found[ref] = 10; // if match detected then add as a match else discard
                                                        nidx_ref[ref] = nidx[0];
                                         }
                                }
                           }
	...
	for (int counter = 0; counter < num_counter; counter++) {
		...
		for (int img_cnt = 0; img_cnt < img_num; img_cnt++) {
			...
			vector<cv::Point2f> matches_kaze, inliers_kaze;
                        ...
                        float bufvalue_ref;
                        bool switchs = false;                                       
                        for(int i=0; i<number_keypoints1; i++){
                                bufvalue = 0; 
                                dist0 = (double) FLT_MAX; 
                                dist1 = (double) FLT_MAX;
                                nidx[0] = -1; 
                                nidx[1] = -1;
                                switchs = false;
                                for(int j=0; j<number_keypoints2; j++){
                                        bufvalue = 0.0; 
                                        for(int k=0; k<64; k++){
                                        	float t = adesc11[i][k] - adesc22[j][k];
                                                bufvalue += (float) t*t; 
                                        }
                                        bufvalue_ref = (float)sqrt(bufvalue); 
                                        if(bufvalue_ref < dist0 ){
                                        	if(switchs) {
                                        		nidx[1] = nidx[0];
                                                	dist1 = dist0;
                        ...
                        cudaMemcpy(cudaDev_adesc22, cuda_adesc22, numBytes, cudaMemcpyHostToDevice); // copy descriptors of Train Image to 
                                                                       //  cudaDev_adesc22
                        const double t5 = (double)(cv::getTickCount());
                        knnMatch<<<((number_keypoints1/1024)),1024>>>( cudaDev_adesc11, cudaDev_adesc22, cudaDev_found, cudaDev_nidx, number_keypoints2, number_keypoints1); // launch with number_of_keypoints1 with each thread calculating the two most minimum distance and 
                                       // check for a match
			...
			if (err != cudaSuccess) 
    				printf("Error: %s\n", cudaGetErrorString(err));
                        ...
                        for(unsigned int i=0; i<number_keypoints1;i++){ // iterate for each key point in train image to check if a match was found
                        	if(cuda_found[i] == 10){ // if found then add the key points between train image and query image as a match
                                	matches_kaze.push_back(key_points11[i].pt);
                                        matches_kaze.push_back(key_points22[cuda_nidx[i]].pt);
                                }
                        }
                        cudaFree(cudaDev_found); // free the memory
